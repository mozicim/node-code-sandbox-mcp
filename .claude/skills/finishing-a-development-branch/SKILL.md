...
Wait for exact "discard" confirmation before proceeding.
...
- **Deleting branch before removing worktree** — `git branch -d` fails when worktree references it
- **Running `git worktree remove` from inside the worktree** — always `cd` to main repo root first
- **Cleaning up harness-owned worktrees** — only clean up Superpowers-created worktrees
